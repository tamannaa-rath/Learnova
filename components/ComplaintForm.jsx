...
export default function ComplaintForm({ onClose, onSubmitComplaint }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!form.title || !form.description || (!isAnonymous && !form.studentName)) return;
    onSubmitComplaint({
      title: form.title,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
          ...
          <div className="lg:col-span-4 bg-gradient-to-br from-indigo-600 via-purple-600 to-violet-700 p-6 md:p-8 text-white flex flex-col justify-between">
            <div>
              ...
              <p className="text-xs uppercase tracking-[0.3em] text-indigo-200 font-semibold">Guidelines</p>
              <h2 className="mt-1 text-2xl font-bold tracking-tight">Filing a Complaint</h2>
              <p className="text-indigo-100 text-xs leading-6 mt-3">
                Please enter your details below. Provide a clear description of your classroom, course, or hostel issue so administration can review it promptly.
              </p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="lg:col-span-8 p-6 md:p-8 space-y-5 text-slate-900 dark:text-slate-100">
              ...
              {!isAnonymous && (
                ...
                >
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs uppercase font-bold tracking-[0.15em] text-slate-400">Your Name</label>
                    <input
                      type="text"
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs uppercase font-bold tracking-[0.15em] text-slate-400">Roll Number</label>
                    <input
                      type="text"
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs uppercase font-bold tracking-[0.15em] text-slate-400">Department</label>
                    <input
                      type="text"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs uppercase font-bold tracking-[0.15em] text-slate-400">Complaint Title</label>
                <input
                  required
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs uppercase font-bold tracking-[0.15em] text-slate-400">Category</label>
                <select
                  value={form.category}
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs uppercase font-bold tracking-[0.15em] text-slate-400">Priority Level</label>
                  ...
                  return (
                    <button
                      ...
                      className={`border px-3 py-2.5 rounded-2xl text-xs font-semibold transition-all cursor-pointer ${
                        isSelected ? "bg-slate-900 text-white dark:bg-white dark:text-slate-950 border-transparent font-bold" : "border-slate-200 dark:border-slate-800 bg-slate-50/10 dark:bg-slate-900/10 text-slate-400 opacity-60 hover:opacity-100"
                      }`}
                    >
                      {p}
                    </button>
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs uppercase font-bold tracking-[0.15em] text-slate-400">Issue Description</label>
              <textarea
                rows={4}
                required
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs uppercase font-bold tracking-[0.15em] text-slate-400">Supporting Files</label>
              <div
              ...
              >
                <input type="file" ref={fileInputRef} className="hidden" onChange={(e) => e.target.files?.[0] && setAttachment(e.target.files[0])} accept="image/*,.pdf" />
                <Upload size={16} className="text-slate-400 mb-1" />
                <p className="text-xs text-slate-400">
                  {attachment ? <span className="text-indigo-600 dark:text-indigo-400 font-mono font-medium flex items-center gap-1"><Check size={12} /> {attachment.name}</span> : "Drag files here, or click to choose from folder"}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-3.5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/20 dark:bg-slate-900/10">
              <div>
                <span className="text-xs font-bold block text-slate-700 dark:text-slate-300">Submit Anonymously</span>
                <span className="text-[11px] text-muted-foreground">Hide your name and student ID from the admin review panel.</span>
              </div>
              <button
                type="button"
              ...
              >
                <motion.div layout animate={{ x: isAnonymous ? 18 : 0 }} transition={{ type: "spring", stiffness: 500, damping: 30 }} className="bg-white dark:bg-slate-900 w-4 h-4 rounded-full shadow-sm" />
              </button>
            </div>

            <Button type="submit" className="w-full py-4 rounded-2xl bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 text-sm font-semibold shadow-md cursor-pointer">
              Submit Complaint
            </Button>
          </form>
        </div>
